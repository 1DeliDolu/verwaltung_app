<?php $title = 'Task erstellen'; ?>
<div class="hero">
    <p class="eyebrow">Neue Aufgabe</p>
    <h1 class="display-6 fw-semibold">Aufgabe erfassen</h1>
    <p class="lead">Lege eine neue Aufgabe in einer sichtbaren Abteilung an und ordne sie bei Bedarf direkt einer Person zu.</p>
</div>
    ...
    <form method="POST" action="/tasks">
        ...
        <div class="row g-3">
            <div class="col-12 col-lg-6">
                ...
                <select class="form-select" id="department_id" name="department_id" required>
                    ...
                    <?php foreach ($departments as $department): ?>
                        <option value="<?= htmlspecialchars((string) $department['id'], ENT_QUOTES, 'UTF-8') ?>" <?= (string) ($old['department_id'] ?? '') === (string) $department['id'] ? 'selected' : '' ?>>
            ...
            <div class="col-12 col-lg-6">
                ...
                <select class="form-select" id="priority" name="priority" required>
                    <?php foreach ($priorities as $priorityKey => $priorityLabel): ?>
                        <option value="<?= htmlspecialchars($priorityKey, ENT_QUOTES, 'UTF-8') ?>" <?= (string) ($old['priority'] ?? 'normal') === $priorityKey ? 'selected' : '' ?>>
            ...
            <div class="col-12 col-lg-6">
                ...
                <input class="form-control" id="due_date" name="due_date" type="date" value="<?= htmlspecialchars((string) ($old['due_date'] ?? ''), ENT_QUOTES, 'UTF-8') ?>">
            </div>
            <div class="col-12">
                <label class="form-label fw-semibold" for="description">Beschreibung</label>
                <textarea class="form-control" id="description" name="description" rows="8" required><?= htmlspecialchars((string) ($old['description'] ?? ''), ENT_QUOTES, 'UTF-8') ?></textarea>
            </div>
        </div>
        <div class="d-flex flex-wrap gap-2 mt-4">
            <button class="btn px-4 py-2" type="submit">Aufgabe speichern</button>
            <a class="btn btn-outline-accent px-4 py-2" href="/tasks">Abbrechen</a>
        </div>
    </form>
</div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const selectedAssignee = <?= json_encode((string) ($old['assigned_to_user_id'] ?? '')) ?>;
