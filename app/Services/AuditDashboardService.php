...
final class AuditDashboardService
{
    ...
    public function build(array $filters): array
    {
        ...
        $events = $this->mergedEvents($audit, $filters);

        return [
            'events' => $events,
            'summary' => $this->summaries($audit, $filters),
            'trend' => $this->dailyTrend($events),
            'actionBreakdown' => $this->actionBreakdown($events),
            'topActors' => $this->topActors($events),
            'failureHeatmap' => $this->failureHeatmap($events),
        ];
    }
    ...
    public function weeklyWindow(?DateTimeImmutable $now = null): array
    {
        $now ??= new DateTimeImmutable('now');
        $dateTo = $now->format('Y-m-d');
        $dateFrom = $now->modify('-6 days')->format('Y-m-d');

        return [
            'date_from' => $dateFrom,
            'date_to' => $dateTo,
            'label' => $dateFrom . ' bis ' . $dateTo,
        ];
    }

    public function overallStats(array $events): array
    {
        $total = count($events);
        $failure = 0;
        ...
        return [
            ...
            'success' => $success,
            'failure' => $failure,
            'failure_rate' => $total > 0 ? (int) round(($failure / $total) * 100) : 0,
        ];
    }

    public function recentFailures(array $events, int $limit = 5): array
    {
        $failures = array_values(array_filter(
            $events,
            static fn (array $event): bool => (string) ($event['outcome'] ?? '') === 'failure'
        ));

        return array_slice($failures, 0, $limit);
    }

    public function dashboardUrl(array $params): string
    {
    ...
    public function eventsAsCsv(array $events): string
    {
    ...
    private function summaries(AuditLogService $audit, array $filters): array
    {
        return [
            'admin_user' => [
                'label' => self::SOURCE_LABELS['admin_user'],
                'count' => count($audit->readAdminUserEvents($this->filtersForSource($filters))),
                'url' => $this->dashboardUrl(['source' => 'admin_user']),
            ],
            'task' => [
                'label' => self::SOURCE_LABELS['task'],
                'count' => count($audit->readTaskWorkflowEvents($this->filtersForSource($filters))),
                'url' => $this->dashboardUrl(['source' => 'task']),
            ],
            'mail' => [
                'label' => self::SOURCE_LABELS['mail'],
                'count' => count($audit->readMailActivityEvents($this->filtersForSource($filters))),
                'url' => $this->dashboardUrl(['source' => 'mail']),
            ],
            'calendar' => [
                'label' => self::SOURCE_LABELS['calendar'],
                'count' => count($audit->readCalendarActivityEvents($this->filtersForSource($filters))),
    ...
    private function mergedEvents(AuditLogService $audit, array $filters): array
    {
        ...
        if ($source === '' || $source === 'admin_user') {
            foreach ($audit->readAdminUserEvents($this->filtersForSource($filters)) as $event) {
                $events[] = [
                    'source' => 'admin_user',
                    'source_label' => self::SOURCE_LABELS['admin_user'],
        ...
        if ($source === '' || $source === 'task') {
            foreach ($audit->readTaskWorkflowEvents($this->filtersForSource($filters)) as $event) {
                $events[] = [
                    'source' => 'task',
                    'source_label' => self::SOURCE_LABELS['task'],
                    'timestamp' => (string) ($event['timestamp'] ?? ''),
                    'action' => (string) ($event['action'] ?? ''),
                    'outcome' => (string) ($event['outcome'] ?? ''),
                    'actor_email' => (string) ($event['actor']['email'] ?? ''),
        ...
        if ($source === '' || $source === 'mail') {
            foreach ($audit->readMailActivityEvents($this->filtersForSource($filters)) as $event) {
                $events[] = [
                    'source' => 'mail',
                    'source_label' => self::SOURCE_LABELS['mail'],
                    'timestamp' => (string) ($event['timestamp'] ?? ''),
                    'action' => (string) ($event['action'] ?? ''),
                    'outcome' => (string) ($event['outcome'] ?? ''),
                    'actor_email' => (string) ($event['actor']['email'] ?? ''),
        ...
        if ($source === '' || $source === 'calendar') {
            foreach ($audit->readCalendarActivityEvents($this->filtersForSource($filters)) as $event) {
                $events[] = [
                    'source' => 'calendar',
                    'source_label' => self::SOURCE_LABELS['calendar'],
                    'timestamp' => (string) ($event['timestamp'] ?? ''),
                    'action' => (string) ($event['action'] ?? ''),
                    'outcome' => (string) ($event['outcome'] ?? ''),
                    'actor_email' => (string) ($event['actor']['email'] ?? ''),
    ...
    private function filtersForSource(array $filters): array
    {
        return [
