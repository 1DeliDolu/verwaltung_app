...
final class PasswordResetRequestThrottleService
{
    ...
    public function recordRequest(string $email, ?string $ipAddress): array
    {
        ...
        $requestAttempts = (int) ($existing['request_attempts'] ?? 0) + 1;
        $windowStartedAt = $existing['window_started_at'] ?? $now;
        $lockedUntil = null;

        if ($requestAttempts >= $this->maxAttempts()) {
    ...
    public function lockoutMessage(int $availableInSeconds): string
    {
        ...
        if ($minutes <= 1) {
            return 'Zu viele Passwort-Reset-Anfragen. Bitte in 1 Minute erneut versuchen.';
        }

        return sprintf('Zu viele Passwort-Reset-Anfragen. Bitte in %d Minuten erneut versuchen.', $minutes);
    }
    ...
    private function activeState(string $email, ?string $ipAddress, bool $clearExpired): ?array
    {
        ...
        return [
            'request_attempts' => (int) ($row['request_attempts'] ?? 0),
    ...
    private function clear(string $email, ?string $ipAddress): void
    {
        $statement = $this->pdo()->prepare(
            'DELETE FROM password_reset_request_limits
             WHERE throttle_key = :throttle_key'
        );
        $statement->execute([
    ...
    private function upsertState(
        ...
        DateTimeImmutable $lastRequestedAt,
        ?DateTimeImmutable $lockedUntil
    ): void {
        $statement = $this->pdo()->prepare(
            'INSERT INTO password_reset_request_limits (
                throttle_key,
                email,
                ip_address,
                request_attempts,
            ...
            ON DUPLICATE KEY UPDATE
                ...
                request_attempts = VALUES(request_attempts),
        ...
        $statement->execute([
            ...
            'last_requested_at' => $lastRequestedAt->format('Y-m-d H:i:s'),
    ...
    {
        return max(1, (int) $this->app->config('auth.password_reset.request_throttle.max_attempts', 3));
    }

    private function decaySeconds(): int
    {
        return max(60, (int) $this->app->config('auth.password_reset.request_throttle.decay_seconds', 900));
    }
